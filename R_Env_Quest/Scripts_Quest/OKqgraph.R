...
##### 1 DATA 
data_full <- read.csv("C:/Users/lisa/Documents/-These/MANIP_Quest/Share/Analyses/Scale_fornetwork2.csv")
#str(Data)
data_full$X = NULL

library(dplyr)
data_full <- data_full %>% mutate(A1_sexe = as.numeric(A1_sexe),
                                      A2_diff = as.numeric(A2_diff),
                                      C_AP = as.numeric(C_AP),
                                      M1_douleurs_nb = as.numeric(M1_douleurs_nb),
                                      sit_eco = as.numeric(sit_eco))
...
gr3 <- list(c(1:4), c(5),c(6:8),
            c(9:10),c(11:14), 
            c(15:18), c(19:21),c(22:28),c(29),c(30),c(31))           

##### 3. Network 1 

### 3.1 Figure 1

pdf("TOT_Subscale_Network.pdf")
graph.g<-qgraph(data.cor, graph="glasso", layout="spring", 
                     vsize=7, cut=0, maximum=.45, sampleSize = nrow(data_full),
...
centralityPlot(graph.g,include="All",orderBy = "Strength")
dev.off()


pdf("TOT_Subscale_Centrality3.pdf")
centralityPlot(graph.g,include="All",orderBy = "Betweenness")
dev.off()


pdf("TOT_Subscale_Clustering.pdf")
